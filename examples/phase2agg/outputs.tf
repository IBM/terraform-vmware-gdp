output "vm_names" {
  ...
  value       = { for k, v in module.aggregator : k => v.vm_name }
}

output "vm_power_states" {
  description = "Power states of all Guardium VMs"
  value       = { for k, v in module.aggregator : k => v.vm_power_state }
}

output "vm_ip_addresses" {
  description = "The IP addresses of all Guardium VMs (from instances.json)"
  value       = { for k, v in module.aggregator : k => [for inst in local.instances : inst.network_interface_ip if inst.vm_name == k][0] }
}

output "vm_details" {
  description = "Complete details of all deployed Guardium VMs"
  value = {
    for k, v in module.aggregator : k => {
      vm_name       = v.vm_name
